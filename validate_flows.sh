...
#
# Checks performed:
#   * record count and packet/byte totals
#   * every preserved record field, byte-for-byte
#   * packet / byte / protocol / source-port / destination-port distributions
#   * per-minute time-series flow/packet/byte volume
#   * source and destination top-talker structure (ranked, addresses omitted)
#   * no original IP remains anywhere in the output
#   * IP pseudonyms are changed, deterministic, and collision-free
#
# Tools: nfdump, coreutils (awk/sort/comm/cmp/paste). Bash only.
...
check_mapping() { # raw-IP-rows anon-IP-rows diagnostics
...
for src in "${input_files[@]}"; do
  ...
  # to preserve. Epoch timestamps (%tsr/%ter) avoid locale/formatting drift.
